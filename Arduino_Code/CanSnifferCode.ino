...
// ================== CSV OUTPUT ==================
static void sendCSV() {
  // Your mini screen expects 4 fields:
  // rudder_raw,rpm_raw,shift_indicator,fuel_gph

  // Map TRIM% (0..100) -> "rudder_raw" (0..4095) so your Pi can reuse rudder gauge
  int rudder_raw = 2047; // safe default centered
  if (millis() - last_trim_ms <= CAN_STALE_MS && trim_percent >= 0) {
    int tp = trim_percent;
    if (tp < 0) tp = 0;
    if (tp > 100) tp = 100;
  ...
  // RPM to "rpm_raw"
  int rpm_raw = (millis() - last_rpm_ms > CAN_STALE_MS) ? 0 : rpm_calibrated;

  // If gear CAN is stale, force Neutral
  if (millis() - last_gear_ms > CAN_STALE_MS) {
    gearCharV = 'N';
  }
  int shift_indicator = gearToShiftIndicator(gearCharV);

  // If fuel is stale, send 0.0
  float fuelOut = (millis() - last_fuel_ms > STALE_FUEL_MS) ? 0.0f : fuel_gph;
